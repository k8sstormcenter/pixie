...
Status LogModule::Init() {
  PX_ASSIGN_OR_RETURN(
      ...
      FuncObject::Create(kTetragonID, {"glob_pattern", "table_name", "ttl"}, {},
                         ...
                         std::bind(TetragonHandler::Eval, mutations_ir_, std::placeholders::_1,
                                   std::placeholders::_2, std::placeholders::_3),
                         ast_visitor()));
  PX_RETURN_IF_ERROR(upsert_fn->SetDocString(kTetragonDocstring));
  AddMethod(kTetragonID, upsert_fn);

  PX_ASSIGN_OR_RETURN(std::shared_ptr<FuncObject> delete_fn,
                      FuncObject::Create(kTetragonID, {"name"}, {},
                                         /* has_variable_len_args */ false,
                                         /* has_variable_len_kwargs */ false,
                                         std::bind(DeleteTetragonHandler::Eval, mutations_ir_,
                                                   std::placeholders::_1, std::placeholders::_2,
                                                   std::placeholders::_3),
                                         ast_visitor()));
  PX_RETURN_IF_ERROR(upsert_fn->SetDocString(kDeleteTetragonDocstring));
  AddMethod(kDeleteTetragonID, delete_fn);

  return Status::OK();
}

StatusOr<QLObjectPtr> TetragonHandler::Eval(MutationsIR* mutations_ir, const pypa::AstPtr& ast,
                                              const ParsedArgs& args, ASTVisitor* visitor) {
  DCHECK(mutations_ir);
  ...
  mutations_ir->CreateTetragonDeployment(glob_pattern_str, table_name_str, ttl_ns);

  return std::static_pointer_cast<QLObject>(std::make_shared<NoneObject>(ast, visitor));
}

StatusOr<QLObjectPtr> DeleteTetragonHandler::Eval(MutationsIR* mutations_ir,
  ...
  mutations_ir->DeleteTetragon(glob_pattern_str);
