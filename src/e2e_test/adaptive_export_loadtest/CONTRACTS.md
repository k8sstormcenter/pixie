...
| C18 | **Dark-vector rows carry pod/namespace attribution.** Tracepoint tables emit a raw kernel pid with no upid, so the `dc_snoop`/`creds_change` export presets resolve `namespace`+`pod` by a **process_stats merge on pid** (`px.upid_to_pid`, the validated PodEnrichPxL join; pid-only, not pid+asid — see compile.go). Best-effort **left** join: blank pod for host/transient pids (correct — they have no pod), so a short-lived process must live long enough to be sampled by process_stats. `stack_trace` needs no merge — `stack_traces.beta` carries upid and resolves via `ctx`. | ✅ in the presets | pid collisions across nodes are a known best-effort limitation of the pid-only join. The creds_change calibration verifies pod resolves to the firing namespace. |
