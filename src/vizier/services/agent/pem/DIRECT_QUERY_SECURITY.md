...
| **alg:none forgery (RFC 8725 §3.1).** Adversary submits unsigned token with `"alg":"none"`. | Verifier requires `alg == "HS256"` in the header; any other value → `UNAUTHENTICATED`. |
...
|---|---|
| **Key compromise.** If `pl-cluster-secrets/jwt-signing-key` leaks, an adversary can mint valid tokens for kelvin / query-broker / direct-query at will. | Same threat model as the entire in-cluster service mesh. Out-of-band protections (RBAC on the Secret, sealed-secrets/SOPS at rest, key rotation) are the appropriate controls. |
