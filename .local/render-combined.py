...
res_re = re.compile(
    r'^\s*(\d+)x\s+achieved\s+http=(-?\d+)\s+redis=(-?\d+)\s+pgsql=(-?\d+)\s+TOTAL=(-?\d+)\s+\|\s+srv-cpu\s+http=(\d+)m\s+redis=(\d+)m\s+pgsql=(\d+)m\s+\|\s+pem=(\d+)m')
...
ax = axes[0, 0]
ax.plot(mults, [rows[m]['target'] / 3 for m in mults], marker='o',
        label='target (per protocol)', color='gray', linestyle='--', linewidth=1.5)
