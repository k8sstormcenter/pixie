...
constexpr char kSingleTetragon[] = R"pxl(
import pxlog

glob_pattern = 'test.json'
pxlog.Tetragon(glob_pattern, 'test_table', '5m')
)pxl";

constexpr char kSingleTetragonProgramPb[] = R"pxl(
glob_pattern: "test.json"
...
class TetragonCompilerTest : public ASTVisitorTest {
 protected:
  StatusOr<std::shared_ptr<compiler::MutationsIR>> CompileTetragonScript(
...
TEST_F(TetragonCompilerTest, parse_single_tetragon) {
  ASSERT_OK_AND_ASSIGN(auto mutation_ir, CompileTetragonScript(kSingleTetragon));
  plannerpb::CompileMutationsResponse pb;
  EXPECT_OK(mutation_ir->ToProto(&pb));
  ASSERT_EQ(pb.mutations_size(), 1);
  EXPECT_THAT(pb.mutations()[0].tetragon(),
              testing::proto::EqualsProto(kSingleTetragonProgramPb));
}
