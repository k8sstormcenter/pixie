...
CREATE TABLE IF NOT EXISTS forensic_db.dc_snoop (
  time_ UInt64, pid Int32, comm String, t String, file String,
  namespace String, pod String, hostname String,
  event_time DateTime64(9) DEFAULT toDateTime64(time_ / 1e9, 9)
) ENGINE = MergeTree ORDER BY (event_time, pod);
...
CREATE TABLE IF NOT EXISTS forensic_db.stack_trace (
  time_ UInt64, upid UInt128, namespace String, pod String, container String,
...
CREATE TABLE IF NOT EXISTS forensic_db.creds_change (
  time_ UInt64, pid Int32, comm String, old_uid UInt32, new_uid UInt32,
  namespace String, pod String, hostname String,
  event_time DateTime64(9) DEFAULT toDateTime64(time_ / 1e9, 9)
) ENGINE = MergeTree ORDER BY (event_time, pod);
