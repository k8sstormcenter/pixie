...
StatusOr<std::unique_ptr<RowBatch>> ClickHouseSourceNode::ConvertClickHouseBlockToRowBatch(
  ...
  for (size_t col_idx = 0; col_idx < num_cols; ++col_idx) {
    ...
    if (type_name == "String" && expected_type == types::DataType::UINT128) {
      ...
      auto builder =
          types::MakeArrowBuilder(types::DataType::UINT128, arrow::default_memory_pool());
      ...
      for (size_t i = 0; i < num_rows; ++i) {
        ...
        if (colon_pos == std::string::npos) {
          return error::InvalidArgument("Invalid UINT128 string format: $0 (expected high:low)",
                                        value);
        }
        ...
        PX_RETURN_IF_ERROR(
            table_store::schema::CopyValue<types::DataType::UINT128>(builder.get(), uint128_val));
      }
        ...
        case types::DataType::UINT128: {
          ...
          for (int i = 0; i < typed_array->length(); i++) {
            if (typed_array->IsNull(i)) {
              ...
              PX_RETURN_IF_ERROR(
                  table_store::schema::CopyValue<types::DataType::UINT128>(builder.get(), val));
            }
          }
          break;
        }
