...
class StirlingTetragonTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    stirling_->RegisterDataPushCallback(
        absl::bind_front(&StirlingTetragonTest::AppendData, this));
  }
  ...
  StatusOr<stirlingpb::Publish> WaitForStatus(sole::uuid trace_id) {
    ...
    do {
      s = stirling_->GetTetragonInfo(trace_id);
  ...
  void DeployTetragon(std::string file_name, bool trigger_stop = true) {
    ...
    if (trigger_stop) {
      ASSERT_OK(stirling_->RemoveTetragon(id));
...
class TetragonJSONTest : public StirlingTetragonTest {
 protected:
  const std::string kFilePath =
      BazelRunfilePath("src/stirling/source_connectors/tetragon/testdata/test.json");
};

TEST_F(TetragonJSONTest, ParsesJSONFile) {
  DeployTetragon(kFilePath);
...
TEST_F(TetragonJSONTest, ContinuesReadingAfterEOFReached) {
  ...
  // TetragonConnector parses the first line to infer the file's schema, an empty file will cause an error.
  ofs << R"({"id": 0, "active": false, "score": 6.28, "name": "item0", "object": {"a": 1, "b": 2}, "arr": [0, 1, 2]})" << std::endl;

  DeployTetragon(file_name, false);
  EXPECT_THAT(record_batches_, SizeIs(1));
  auto& rb = record_batches_[0];
  // Expect there to be 8 columns. time_ and the 4 cols from the JSON file.
  EXPECT_EQ(rb->size(), 8);
...
TEST_F(TetragonJSONTest, ContinuesReadingAfterFileRotation) {
