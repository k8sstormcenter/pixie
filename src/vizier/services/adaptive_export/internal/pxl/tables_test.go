...
// (V7), stack_trace (V9) — canonical schemas, active. The six reserved dx_*
// tracepoint tables (dx_vfs_events/dx_unlink/dx_dlookup/dx_mprotect/dx_bpf/
// dx_ptrace) were removed: no bpftrace was ever deployed for them, so every
// steer-all query on them could only fail PxL compile. Update if the spec changes.
