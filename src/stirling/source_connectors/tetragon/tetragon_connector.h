...
class TetragonConnector : public SourceConnector {
 ...
 public:
  ...
  static StatusOr<std::unique_ptr<TetragonConnector> > Create(std::string_view source_name,
                                                              const std::filesystem::path input_file_name);

  TetragonConnector() = delete;
  ~TetragonConnector() = default;
  Status InitImpl() override;
  Status StopImpl() override;
  void TransferDataImpl(ConnectorContext* ctx) override;

 protected:
  explicit TetragonConnector(std::string_view source_name, const std::filesystem::path& input_file_name,
                               std::ifstream file,
