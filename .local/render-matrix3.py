...
def parse_summary_line(line):
    ...
    if m: out['k6_actual'] = int(m.group(1))
    m = re.search(r'target=(\d+)/s', line)
    if m: out['target'] = int(m.group(1))
...
for run in runs:
    ...
    fig.suptitle(f"{run['name']}  k6={run.get('k6_actual','?')}/s  target={run.get('target','?')}/s  ({run['replicas']}×{run['qps_per_pod']})", fontsize=11)
    fig.tight_layout()
...
ax.set_ylabel('req/sec'); ax.set_title('k6 achieved vs target'); ax.legend(); ax.grid(True, alpha=0.3)

ax = axes[0,1]
ax.bar(x, delivery, color='tab:green')
ax.set_xticks(x); ax.set_xticklabels(names, rotation=20, ha='right')
ax.set_ylabel('% of target'); ax.set_title('Delivery ratio (k6 / target)')
...
ax.set_xticks(x); ax.set_xticklabels(names, rotation=20, ha='right')
ax.set_ylabel('count'); ax.set_title('Peak nf_conntrack_count (cap: 1,048,576)')
...
ax.set_xticks(x); ax.set_xticklabels(names, rotation=20, ha='right')
ax.set_ylabel('millicores'); ax.set_title('vizier-PEM CPU')
ax.legend(); ax.grid(True, alpha=0.3)

ax = axes[2,0]
ax.bar(x, coredns, color='tab:brown')
ax.set_xticks(x); ax.set_xticklabels(names, rotation=20, ha='right')
ax.set_ylabel('queries/sec'); ax.set_title('CoreDNS query rate')
ax.grid(True, alpha=0.3)
...
ax.set_xlabel('loadgen pod count'); ax.set_ylabel('k6 achieved req/sec')
